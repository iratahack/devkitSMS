...
void main(void)
{
    static uint8_t spriteX = (SCREEN_WIDTH - SPRITE_WIDTH) / 2;
    static uint8_t spriteY = (SCREEN_HEIGHT - SPRITE_HEIGHT) / 2;
    ...
    for (;;)
    {
        ...
        // Up or Down?
        if ((keyStatus & PORT_A_KEY_UP) && spriteY > 0)
            spriteY -= 2;
        else if ((keyStatus & PORT_A_KEY_DOWN) && spriteY < SCREEN_HEIGHT - SPRITE_HEIGHT)
            spriteY += 2;

        // Left or Right?
        if ((keyStatus & PORT_A_KEY_LEFT) && spriteX > 0)
            spriteX -= 2;
        else if ((keyStatus & PORT_A_KEY_RIGHT) && spriteX < SCREEN_WIDTH - SPRITE_WIDTH)
            spriteX += 2;
